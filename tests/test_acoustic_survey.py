...
class TestASA(unittest.TestCase):
    ...
    def test_nmf(self):
        ds = self.asa.source_separation(
            window_time=1.0, n_sources=15, save_path=None, verbose=verbose
        )

    def test_features(self):
        self.asa.evolution_multiple(
            method_list=fast_features, band_list=band_list
        )
        ...
        assert np.logical_and(
            ds.zcr_avg.values >= 0, ds.zcr_avg.values <= 1
        ).all()
    ...
    def test_third_oct(self):
        ds = self.asa.evolution_freq_dom(
            "spectrogram", band=third_octaves, db=True
        )
        print(ds)
    ...
    def test_millidecade_bands(self):
        ...
        assert (
            milli_psd_halfhz.frequency[2] - milli_psd_halfhz.frequency[1]
        ) == 0.5
        ...
        if verbose:
            ...
            milli_psd_2hz["millidecade_bands"].mean(dim="id").plot(
                ax=ax, label="2Hz"
            )
            milli_psd_512["millidecade_bands"].mean(dim="id").plot(
                ax=ax, label="512"
            )
            plt.legend()
            plt.show()
    ...
    def test_update_freq_cal(self):
        ...
        ds_psd_updated = self.asa.update_freq_cal(
