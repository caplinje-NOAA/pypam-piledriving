...
class TestAcuFile(unittest.TestCase):
    def setUp(self) -> None:
        self.acu_file = AcuFile(
            "tests/test_data/67416073.210610033655.wav", soundtrap, 1
        )

    @skip_unless_with_plots()
    def test_plots(self):
        self.acu_file.plot_spectrum_per_chunk(scaling="density", db=True)
        self.acu_file.plot_spectrum_median(
            scaling="spectrum", db=True, binsize=10
        )
    ...
    def test_nmf(self):
        self.acu_file.source_separation(
            window_time=1.0,
            n_sources=15,
            binsize=None,
            save_path=None,
