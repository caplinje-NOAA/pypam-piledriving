...
class Signal:
    ...
    def downsample(self, new_fs, filt=None):
        ...
        self.signal = sig.resample_poly(
            self.signal, up=ratio_up, down=ratio_down
        )
    ...
    def sel(self, db=True, energy_window=None, **kwargs):
        """
        ...
        db : bool
            If set to True the result will be given in db, otherwise in uPa
        energy_window: float
            If provided, calculate the sel over the given energy window (e.g. RMS_90
            for energy_window= .9).
        """
        if energy_window:
            [start, end] = utils.energy_window(self.signal, energy_window)

            y = utils.sel(self.signal[start:end], self.fs)
        else:
    ...
    def third_octave_levels(self, db=True, **kwargs):
        ...
        spg : numpy array
            Level of each band
        """
        return self.octave_levels(db, 3, **kwargs)

    def octave_levels(self, db=True, fraction=1, SEL=False, **kwargs):
        """
    ...
    def spectrum(
        ...
        **kwargs
        ...
        if self.psd is None or force_calc:
            self._spectrum(
                scaling=scaling, nfft=nfft, db=db, overlap=overlap, **kwargs
            )
    ...
    def spectrum_slope(
        self, scaling="density", nfft=512, db=True, overlap=0, **kwargs
    ):
        """
    ...
    def aei(
        self, db_threshold=-50, freq_step=100, nfft=512, overlap=0, **kwargs
    ):
        """
        ...
        if slopes is not None:
            ...
            peaks_indices = (
                np.r_[
                    False,
                    meanspec[1:]
                    > np.array([x + slopes[0] for x in meanspec[:-1]]),
                ]
                & np.r_[
                    meanspec[:-1]
                    > np.array([y + slopes[1] for y in meanspec[1:]]),
                    False,
                ]
            )
            ...
            if len(np.intersect1d(consecutiveIndices, peaks_indices)) > 1:
                ...
                maxi = maxi[
                    np.argmax(
                        [
                            meanspec[f]
                            for f in np.intersect1d(
                                consecutiveIndices, peaks_indices
                            )
                        ]
                    )
                ]
    ...
    def reduce_noise(self, nfft=512, verbose=False):
        ...
        """
        s = nr.reduce_noise(
            y=self.signal, sr=self.fs, n_fft=nfft, win_length=nfft
        )
        if verbose:
            db = True
            label, units = output_units.get_units(
                "spectrogram", log=db, p_ref=1.0
            )
    ...
    def plot(
        ...
        label, units = output_units.get_units(
            "spectrum_" + scaling, log=db, p_ref=1.0
        )
        label_a, units_a = output_units.get_units("amplitude", log=False)
...
class Blocks:
    ...
    def __next__(self):
        ...
        if (self.n * self.blocksize) < self.nsamples:
            block = self.signal[
                self.n * self.blocksize : self.n * self.blocksize
                + self.blocksize
            ]
            self.n += 1
